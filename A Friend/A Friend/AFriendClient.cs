    ...
    class AFriendClient
    {
        ...
        public static void ExecuteClient(object obj)
        {
            try
            {
                ...
                user = null;
                client.Send(Encoding.ASCII.GetBytes("2004"));
        ...
        public static bool Logged_in(string tk, string mk)
        {
            ...
            try
            {
                ...
                client.Send(Encoding.ASCII.GetBytes("0010" + tk + " " + mk)); //0010 = log in
                Receive_from_id(client);
                if (user == null)
                {
                    client.Send(Encoding.ASCII.GetBytes("2004")); // 2004 = stop client
                    client.Shutdown(SocketShutdown.Both);
                    client.Close();
                    return false;
                }
        ...
        public static void Send_to_id(Socket self, string id, string myid, string str)
        {
            ...
            try
            {
                self.Send(Encoding.ASCII.GetBytes(sent_message));
            }
            catch (Exception e)
            {
                Console.WriteLine(e.StackTrace);
        ...
        public static bool Signed_up(string tk, string mk)
        {
            try
            {
                ...
                client = new Socket(ipAddr.AddressFamily, SocketType.Stream, ProtocolType.Tcp);
                client.Connect("mancitiss.ddns.net", 11111);
                client.Send(Encoding.ASCII.GetBytes("0011" + tk + " " + mk)); //0011 = sign up
                Receive_from_id(client);
                if (instruction == "1011")
                {
                    success = true;
